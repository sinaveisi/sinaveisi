<h1 align="center">Hi, I'm Sina Veisi 👋</h1>
<h3 align="center">Medical Informatics researcher building AI for your Health and safety</h3>

<p align="center">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=18&pause=1000&color=2E9EF7&center=true&vCenter=true&width=600&lines=MSc+Medical+Informatics+%E2%80%94+Urmia+University+of+Medical+Sciences;Building+multimodal+AI+for+ECG+interpretation;7+years+Android+dev+before+I+went+full+research;Chasing+a+PhD+in+Health+%26+Biomedical+Informatics" alt="Typing SVG" />
</p>

---

### 🔬 What I'm working on

**Medical Informatics** Specialist.

I spent 7 years as a freelance Android and Wear OS developer. I know what it takes to ship an app to a phone or smartwatch. Today, I bring that hardcore engineering mindset to my real passion: AI that actually ships to a clinical setting.

My focus has shifted from building standard mobile apps to engineering the underlying intelligent systems that power them. I bridge the gap between clinical research and deployable software.

### 🧠 Research interests

- Mobile and Wearable Health Technologies
- Artificial Intelligence and Machine Learning for Health Systems
- AI Engineering & Automation and Real-World AI Evaluation
- AI for Healthcare Cybersecurity

### 🎓 Currently

- 📄 Published in *BMC Medical Education*, with several manuscripts in preparation
- 🩺 Building and evaluating an ECG mHealth app using a pre-post quasi-experimental design
- 🌍 Looking for PhD positions in Health & Biomedical Informatics - Digital Health
- 🧑‍🏫 Teaching assistant across three courses at Urmia and University of Medical Sciences

### 🛠️ Tech I use

<p align="left">
  <img src="https://img.shields.io/badge/Kotlin-7F52FF?style=flat-square&logo=kotlin&logoColor=white" />
  <img src="https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white" />
  <img src="https://img.shields.io/badge/PyTorch-EE4C2C?style=flat-square&logo=pytorch&logoColor=white" />
  <img src="https://img.shields.io/badge/Android-3DDC84?style=flat-square&logo=android&logoColor=white" />
  <img src="https://img.shields.io/badge/Linux-FCC624?style=flat-square&logo=linux&logoColor=black" />
</p>

### ♟️ Off the clock

FIDE-rated chess player (~2000). Native Persian, C1 English. I've also self-hosted infrastructure to get around network censorship, if that tells you anything about how I approach a hard problem.

### 📫 Reach me

<p align="left">
  <!-- swap these placeholders for your real links -->
  <a href="mailto:sina.weisy1@gmail.com"><img src="https://img.shields.io/badge/Email-D14836?style=flat-square&logo=gmail&logoColor=white" /></a>
  <a href="https://linkedin.com/in/sinaveisi"><img src="https://img.shields.io/badge/LinkedIn-0077B5?style=flat-square&logo=linkedin&logoColor=white" /></a>
</p>
